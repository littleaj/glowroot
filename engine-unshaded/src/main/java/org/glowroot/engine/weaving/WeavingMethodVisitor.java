...
import org.glowroot.engine.bytecode.api.Bytecode;
...
import org.glowroot.engine.weaving.Advice.AdviceParameter;
import org.glowroot.engine.weaving.Advice.ParameterKind;
...
class WeavingMethodVisitor extends AdviceAdapter {
    ...
    private static final Method parameterHolderImplCreateMethod = new Method("create",
            "(Ljava/lang/Object;)Lorg/glowroot/instrumentation/api/internal/ParameterHolderImpl;");
    ...
    private void loadOptionalReturnValue(int opcode, boolean dup) {
        if (opcode == RETURN) {
            // void
            visitMethodInsn(INVOKESTATIC, "org/glowroot/engine/bytecode/api/VoidReturn",
                    "getInstance", "()Lorg/glowroot/instrumentation/api/weaving/OptionalReturn;",
                    false);
        } else {
            loadReturnValue(opcode, dup, true);
            visitMethodInsn(INVOKESTATIC, "org/glowroot/engine/bytecode/api/NonVoidReturn", "create",
                    "(Ljava/lang/Object;)Lorg/glowroot/instrumentation/api/weaving/OptionalReturn;",
                    false);
        }
    }
