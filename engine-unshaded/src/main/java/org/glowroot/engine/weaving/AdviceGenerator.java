...
class AdviceGenerator {
    ...
    private AdviceGenerator(AdviceConfig config, @Nullable String instrumentationId,
        ...
        adviceInternalName = "org/glowroot/engine/weaving/GeneratedAdvice" + uniqueNum;
        if (config.isTraceEntryOrGreater() || !config.transactionNameTemplate().isEmpty()
            ...
            methodMetaInternalName = "org/glowroot/engine/weaving/GeneratedMethodMeta" + uniqueNum;
        } else {
    ...
    private void addClassAnnotation(ClassWriter cw) {
        AnnotationVisitor annotationVisitor =
                cw.visitAnnotation("Lorg/glowroot/instrumentation/api/weaving/Pointcut;", true);
    ...
    private void addStaticInitializer(ClassWriter cw) {
        ...
        if (config.isTimerOrGreater()) {
            ...
            mv.visitMethodInsn(INVOKESTATIC, "org/glowroot/instrumentation/api/Agent",
                    "getTimerName",
                    "(Ljava/lang/Class;)Lorg/glowroot/instrumentation/api/TimerName;", false);
        ...
        if (!config.enabledProperty().isEmpty() && instrumentationId != null) {
            ...
            mv.visitMethodInsn(INVOKEINTERFACE,
                    "org/glowroot/instrumentation/api/config/ConfigService", "getBooleanProperty",
    ...
    private void addIsEnabledMethodIfNeeded(ClassWriter cw, boolean checkNotInTransaction,
        ...
        visitAnnotation(mv, "Lorg/glowroot/instrumentation/api/weaving/IsEnabled;");
        mv.visitCode();
        if (checkNotInTransaction && !checkPropertyNotEnabled) {
            mv.visitVarInsn(ALOAD, 0);
            mv.visitMethodInsn(INVOKEINTERFACE,
                    "org/glowroot/instrumentation/api/OptionalThreadContext", "isInTransaction",
                    "()Z",
                    true);
    ...
    private void addOnBeforeMethod(ClassWriter cw) {
        MethodVisitor mv = visitOnBeforeMethod(cw, "Lorg/glowroot/instrumentation/api/TraceEntry;");
        mv.visitCode();
        if (!config.traceEntryEnabledProperty().isEmpty() && instrumentationId != null) {
        ...
        if (config.isTransaction()) {
            ...
            mv.visitMethodInsn(INVOKEVIRTUAL, methodMetaInternalName, "getTransactionNameTemplate",
                    "()Lorg/glowroot/engine/bytecode/api/MessageTemplate;", false);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitVarInsn(ALOAD, 2);
            mv.visitVarInsn(ALOAD, 3);
            mv.visitMethodInsn(INVOKESTATIC, "org/glowroot/engine/bytecode/api/Bytecode",
                    "getMessageText",
                    "(Lorg/glowroot/engine/bytecode/api/MessageTemplate;"
        ...
        if (config.isTransaction()) {
            ...
            mv.visitFieldInsn(GETSTATIC,
                    ...
                    "Lorg/glowroot/instrumentation/api/OptionalThreadContext"
                            + "$AlreadyInTransactionBehavior;");
            mv.visitMethodInsn(INVOKEINTERFACE,
                    "org/glowroot/instrumentation/api/OptionalThreadContext", "startTransaction",
                    "(Ljava/lang/String;Ljava/lang/String;"
                            + "Lorg/glowroot/instrumentation/api/MessageSupplier;"
                            + "Lorg/glowroot/instrumentation/api/TimerName;"
                            + "Lorg/glowroot/instrumentation/api/OptionalThreadContext"
                            + "$AlreadyInTransactionBehavior;)"
                            + "Lorg/glowroot/instrumentation/api/TraceEntry;",
                    true);
        } else {
            mv.visitMethodInsn(INVOKEINTERFACE, "org/glowroot/instrumentation/api/ThreadContext",
                    "startTraceEntry",
    ...
    private void addOnBeforeMethodTimerOnly(ClassWriter cw) {
        MethodVisitor mv = visitOnBeforeMethod(cw, "Lorg/glowroot/instrumentation/api/Timer;");
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
        mv.visitFieldInsn(GETSTATIC, adviceInternalName, "timerName",
                "Lorg/glowroot/instrumentation/api/TimerName;");
    ...
    private MethodVisitor visitOnBeforeMethod(ClassWriter cw, String returnInternalName) {
        ...
        visitAnnotation(mv, "Lorg/glowroot/instrumentation/api/weaving/OnBefore;");
        if (methodMetaInternalName != null) {
            checkNotNull(mv.visitParameterAnnotation(1,
                    "Lorg/glowroot/instrumentation/api/weaving/BindReceiver;", true)).visitEnd();
            checkNotNull(mv.visitParameterAnnotation(2,
                    "Lorg/glowroot/instrumentation/api/weaving/BindMethodName;", true)).visitEnd();
            checkNotNull(mv.visitParameterAnnotation(3,
                    "Lorg/glowroot/instrumentation/api/weaving/BindParameterArray;", true))
                            .visitEnd();
            checkNotNull(mv.visitParameterAnnotation(4,
                    "Lorg/glowroot/instrumentation/api/weaving/BindMethodMeta;", true)).visitEnd();
        }
        return mv;
    }
    ...
    private void addOnReturnMethod(ClassWriter cw) {
        ...
        MethodVisitor mv = cw.visitMethod(ACC_PUBLIC + ACC_STATIC, "onReturn",
                "(Lorg/glowroot/instrumentation/api/weaving/OptionalReturn;" + travelerType + ")V",
                null, null);
        checkNotNull(mv.visitParameterAnnotation(0,
                "Lorg/glowroot/instrumentation/api/weaving/BindOptionalReturn;", true)).visitEnd();
        checkNotNull(mv.visitParameterAnnotation(1,
                "Lorg/glowroot/instrumentation/api/weaving/BindTraveler;", true)).visitEnd();
        int travelerParamIndex = 1;
        visitAnnotation(mv, "Lorg/glowroot/instrumentation/api/weaving/OnReturn;");
        mv.visitCode();
        if (!config.traceEntryEnabledProperty().isEmpty()) {
            ...
            mv.visitTypeInsn(INSTANCEOF, "org/glowroot/instrumentation/api/TraceEntry");
            Label label = new Label();
            mv.visitJumpInsn(IFNE, label);
            mv.visitVarInsn(ALOAD, travelerParamIndex);
        ...
        mv.visitMethodInsn(INVOKESTATIC, "org/glowroot/engine/bytecode/api/Bytecode",
                ...
                "(Lorg/glowroot/instrumentation/api/TraceEntry;Ljava/lang/Object;)V", false);
        mv.visitVarInsn(ALOAD, travelerParamIndex);
        Integer stackTraceThresholdMillis = config.traceEntryStackThresholdMillis();
        if (stackTraceThresholdMillis == null) {
            mv.visitMethodInsn(INVOKEINTERFACE, "org/glowroot/instrumentation/api/TraceEntry",
                    "end",
                    "()V", true);
        } else {
    ...
    private void addOnThrowMethod(ClassWriter cw) {
        MethodVisitor mv = cw.visitMethod(ACC_PUBLIC + ACC_STATIC, "onThrow",
                "(Ljava/lang/Throwable;Lorg/glowroot/instrumentation/api/TraceEntry;)V", null,
                null);
        visitAnnotation(mv, "Lorg/glowroot/instrumentation/api/weaving/OnThrow;");
        checkNotNull(mv.visitParameterAnnotation(0,
                "Lorg/glowroot/instrumentation/api/weaving/BindThrowable;", true)).visitEnd();
        checkNotNull(mv.visitParameterAnnotation(1,
                "Lorg/glowroot/instrumentation/api/weaving/BindTraveler;", true)).visitEnd();
        mv.visitCode();
        if (!config.traceEntryEnabledProperty().isEmpty()) {
    ...
    private LazyDefinedClass generateMethodMetaClass(AdviceConfig config) {
        ...
        cw.visitField(ACC_PRIVATE + ACC_FINAL, "messageTemplate",
                "Lorg/glowroot/engine/bytecode/api/MessageTemplate;", null, null).visitEnd();
    ...
    private void addCodeForSetTransactionX(MethodVisitor mv, String templateGetterName,
        ...
        mv.visitMethodInsn(INVOKESTATIC, "org/glowroot/engine/bytecode/api/Bytecode",
                "getMessageText",
                "(Lorg/glowroot/engine/bytecode/api/MessageTemplate;Ljava/lang/Object;"
    ...
    private void generateMethodMetaGetter(ClassWriter cw, String fieldName, String methodName) {
        MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, methodName,
                "()Lorg/glowroot/engine/bytecode/api/MessageTemplate;", null, null);
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
    ...
    private static void addOnAfterMethodTimerOnly(ClassWriter cw) {
        ...
        visitAnnotation(mv, "Lorg/glowroot/instrumentation/api/weaving/OnAfter;");
        checkNotNull(mv.visitParameterAnnotation(0,
                "Lorg/glowroot/instrumentation/api/weaving/BindTraveler;", true)).visitEnd();
        mv.visitCode();
        mv.visitVarInsn(ALOAD, 0);
        mv.visitMethodInsn(INVOKEINTERFACE, "org/glowroot/instrumentation/api/Timer", "stop", "()V",
                true);
        mv.visitInsn(RETURN);
        mv.visitMaxs(0, 0);
        mv.visitEnd();
